...
function sp_ev_save_video($video) {
    ...
    $category_id = get_cat_ID(__('External Videos', 'external-videos'));
...
function sp_ev_settings_page() {
    ...
    if ($_POST['external_videos'] == 'Y' ) {
        ...
        elseif ($_POST['action'] == 'delete_author') {
            if (!sp_ev_local_author_exists($_POST['host_id'], $_POST['author_id'], $options['authors'])) {
                ...
                ?><div class="updated"><p><strong><?php printf(_n('Deleted author and his %d video.', 'Deleted author and his %d videos.', $del_videos, 'external-videos'), $del_videos); ?></strong></p></div><?php
            }
        }
        elseif ($_POST['action'] == 'sp_ev_update_videos') {
            $num_videos = sp_ev_update_videos($options['authors']);
            ?><div class="updated"><p><strong><?php printf(_n('Found %d video.', 'Found %d videos.', $num_videos, 'external-videos'), $num_videos); ?></strong></p></div><?php
        }
        elseif ($_POST['action'] == 'sp_ev_delete_videos') {
            $num_videos = sp_ev_delete_videos();
            ?><div class="deleted"><p><strong><?php printf(_n('Deleted %d video.', 'Deleted %d videos.', $num_videos, 'external-videos'), $num_videos); ?></strong></p></div><?php
        }
    ...
    <h3><?php _e('Add Authors', 'external-videos'); ?></h3>
    <form method="post" action="<?php echo $_SERVER["REQUEST_URI"]; ?>">
        ...
        <p>
            <?php _e('Author ID:', 'external-videos'); ?>
...
function sp_external_videos_columns($columns){
    $columns = array(
        ...
        'parent'      => _x('Attached to', 'column name'),
        'tags'        => __('Tags'),
